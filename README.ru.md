...
- **Не переписывает вслепую**: нет перезаписи, если `rtk` не установлен, команда содержит пайп/редирект/сабшелл/кавычки (`|`, `;`, `>`, `<`, `$`, `()`), уже с префиксом или под обходом — **кроме** простой цепочки `A && B`, где каждый dev-сегмент оборачивается (`ulimit … && rtk npm install`).
