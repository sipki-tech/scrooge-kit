...
- команда содержит пайп, редирект, сабшелл или кавычки (`|`, `;`, `>`, `<`, `$`, `()`) или многострочная — **кроме** простой цепочки `A && B`, где каждый dev-сегмент оборачивается (`ulimit … && rtk npm install`)
